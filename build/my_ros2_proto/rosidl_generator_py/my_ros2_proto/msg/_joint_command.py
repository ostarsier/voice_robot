...
class Metaclass_JointCommand(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            module = import_type_support('my_ros2_proto')
        except ImportError:
            import logging
            import traceback
            logger = logging.getLogger(
                'my_ros2_proto.msg.JointCommand')
            logger.debug(
...
class JointCommand(metaclass=Metaclass_JointCommand):
    ...
    def __init__(self, **kwargs):
        ...
        self.stiffness = array.array('d', kwargs.get('stiffness', []))
    ...
    def stiffness(self, value):
        if isinstance(value, array.array):
            assert value.typecode == 'd', \
                "The 'stiffness' array.array() must have the type code of 'd'"
        ...
        if __debug__:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 all(not (val < -1.7976931348623157e+308 or val > 1.7976931348623157e+308) or math.isinf(val) for val in value)), \
                "The 'stiffness' field must be a set or sequence and each value of type 'float' and each double in [-.000000, .000000]"
        self._stiffness = array.array('d', value)
